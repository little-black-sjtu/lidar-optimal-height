...
def transform_annotations_to_kitti_format(annos, map_name_to_kitti=None, info_with_fakelidar=False, box_range=None):
    """
    Args:
        annos:
    ...
    for anno in annos:
        ...
        if box_range is not None:
            in_range_indices = box_utils.mask_boxes_outside_range_numpy(gt_boxes_lidar, box_range)
            gt_boxes_lidar = gt_boxes_lidar[in_range_indices, :]
            anno['name'] = anno['name'][in_range_indices]
            if 'score' in anno:
                anno['score'] = anno['score'][in_range_indices]
